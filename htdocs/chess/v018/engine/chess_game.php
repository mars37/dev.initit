...
class ChessGame {
    ...
    public function createNewGame($human_color) {
        ...
        $this->game_state->setPosition($this->getInitPosition());
        ...
        $this->game_state->m_crossed_field = null;
    ...
    public function makeMove($cell_index_from, $cell_index_to, $to_figure_string=null) {
        $move_generator = new MoveGenerator($this->game_state);
        $available_moves = $move_generator->generateAllMovesFlat();
        $m_from = (int)(1 << (63 - $cell_index_from));
        $m_to = (int)(1 << (63 - $cell_index_to));
        $found_move = false;
        foreach ($available_moves as $move) {
            if ($move[0] == $m_from && $move[1] == $m_to) {
                $found_move = $move;
                break;
            }
        }
        if ($found_move === false) {
            $this->game_state->text_state = 'Недопустимый ход';
            return;
        }
        ...
        $this->game_state->makeMove($found_move, $to_figure);
        // изменяем информацию о предыдущих полях "откуда" и "куда"
        $this->game_state->prev_move_from = $cell_index_from;
        $this->game_state->prev_move_to = $cell_index_to;

        // изменяем текстовое описание состояния игры
        if ($this->game_state->human_color == $this->game_state->current_player_color) {
